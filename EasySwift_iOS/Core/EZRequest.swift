...
open class EZRequest: NSObject {
    ...
    open var response: DataResponse<(AnyObject, NSError)>? // 获取字符串数据
    ...
    // download下载相关参数
    open var downloadUrl: URLConvertible = ""// 下载图片URL
    ...
    open var method = HTTPMethod.get // 提交方式
    open var parameterEncoding = URLEncoding.queryString // 编码方式 Http头参数设置
    ...
    open var cacheKey: String {
        ...
        } else if !isEmpty(self.requestParams as NSObject) {
    ...
    open var manager: SessionManager {
        get {
            if let reqManager = objc_getAssociatedObject(self, &managerHandle) as? SessionManager {
                return reqManager
                ...
                let aManager = Alamofire.SessionManager(configuration: configuration)
